...
class OneFormer(nn.Module):
    ...
    def instance_inference(self, mask_cls, mask_pred, task_type):
        ...
        #########################################################################################################
        ####### to make sure bounding boxes are always generated this processing is pulled out from 
        ####### if conditional loop
        # to generate bbox around predicted results 
        result.pred_boxes = BitMasks(mask_pred > 0).get_bounding_boxes()
        #########################################################################################################


        ## comment the if conditioning
        # if self.detection_on:
